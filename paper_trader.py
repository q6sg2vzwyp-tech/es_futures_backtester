...
"""
ES Paper Trader (IBKR + ib_insync) — Full Algo Build (2025-11-06)
...
- Robust RT→Polling fallback (5s hist bars when RT starved)
...
class MetaLearner:
    ...
    def _bucket(self, atr_pct: float, trend: float) -> Tuple[int,int]:
        # atr_pct e.g., ATR/close: 0:(<0.6%), 1:(0.6–1.0%), 2:(>1.0%)
        vb = 0
...
class Trader:
    ...
    def connect(self):
        ...
        self.ib = IB()
        try:
            self.ib.connect(self.args.host, self.args.port, clientId=self.args.clientId)
        except Exception as e:
            print("[WARN] IB connect failed:", e)
            self.ib = None
            return
    ...
    def place_oco(self, side: str, qty: int, entry: float, stop: float, tp: float) -> None:
        if not self.args.place_orders or not self.ib or not self.contract:
            ...
            return
        try:
            # Simplified: send market entry then attach stop/limit
            entry_order = MarketOrder(action='BUY' if side=='long' else 'SELL', totalQuantity=qty)
            trade: Trade = self.ib.placeOrder(self.contract, entry_order)
            # attach OCO children (stop + take-profit)
            stop_order = StopOrder(action='SELL' if side=='long' else 'BUY', totalQuantity=qty, auxPrice=stop)
            tp_order   = LimitOrder(action='SELL' if side=='long' else 'BUY', totalQuantity=qty, lmtPrice=tp)
            # NOTE: True OCO linking varies by broker; ib_insync supports bracketOrder helper
            # Here we emulate by submitting a bracket and letting the first fill cancel siblings
            brk = self.ib.bracketOrder(parent=entry_order, takeProfit=tp_order, stopLoss=stop_order)
            for o in brk:
    ...
    def _heartbeat(self, t: dt.datetime, in_window: bool):
        hb = {
            ...
            "rt_age_sec": None if not self.last_rt else (t - self.last_rt).total_seconds(),
